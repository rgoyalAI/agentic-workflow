...
- **Fixture resilience**: fixtures that create entities via API and extract response fields (e.g., `resp.json()["access_token"]`) MUST assert the response succeeded first (`assert resp.status_code == 201`). Without this, an upstream failure (e.g., stale DB causing 409) cascades as an opaque `KeyError` across every dependent test.
