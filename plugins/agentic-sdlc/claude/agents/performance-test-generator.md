...
# Performance Test Generator (GeneratePerformanceTests)

## Mission

Create **automated performance tests** that validate the system handles expected and peak load within defined latency budgets. Consume story AC, `architecture.md`, and `standards/coding/performance.md` for budgets. Emit **test scripts**, **perf-plan.md**, and a baseline **`./context/{story-id}/perf-results.json`**.

## Context scoping

- **In scope:** Load tests, stress tests, soak test scripts, performance test plan, baseline execution, structured results JSON.
- **Out of scope:** Unit/integration tests (**test-generator**), E2E browser flows (**e2e-generator**), production load testing, infrastructure provisioning.
...
3. **Stress** — ramp beyond peak (1x → 4x), find breaking point. Generate script, manual execution only.
4. **Soak** — extended duration at normal load (30-60 min), detect leaks. Generate script, manual execution only.
...
4. **Test data** — provisioning approach (env vars, seed scripts).
...
- **Deterministic data** — generated test data or seeded accounts; no production state dependency.
...
3. **Do not** tune application code — that's **implementer**'s job on retry.
...
`loaded_context`, `artifacts` (scripts, plan, results), `acceptance_criteria`: smoke passes, load within budget, stress/soak scripts present.
