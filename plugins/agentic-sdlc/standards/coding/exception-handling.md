...
| Python | Exception classes | Custom exception hierarchy, `try/except/else/finally`, `@retry` decorators for transient failures, `raise from` for chained context | Bare `except:`, `except Exception as e: pass`, `except ValueError` at API boundaries (catches unrelated library errors), returning `None` for errors |
